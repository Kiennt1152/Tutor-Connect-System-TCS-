...
public class PlatformAdminIntentRule implements IntentRule {
    ...
    public ClassificationDetail classify(String normalized, String lower) {
        boolean isListQuery = normalized.contains("danh sach") || normalized.contains("tim lop") || normalized.contains("xem lop") || normalized.contains("loc ");

        if (!isListQuery && containsAny(normalized,
                ...
                "co bao nhieu lop hoc", "bao nhieu lop hoc", "so luong lop hoc", "bao nhieu lop dang mo", "so luong lop dang mo",
