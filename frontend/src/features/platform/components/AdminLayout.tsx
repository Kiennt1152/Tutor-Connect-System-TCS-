import { Link, NavLink } from 'react-router-dom';
import type { ReactNode } from 'react';
import { AppLogo } from '../../../shared/components/AppLogo';
import { LogoutButton } from '../../../shared/components/LogoutButton';
import { useAuth } from '../../../shared/auth/AuthProvider';import { APP_ROUTES } from '../../../shared/constants/routes';
import {
  IconDashboard,
  IconFlag,
  IconShield,
  IconUser,
  IconUsers,
} from './AdminIcons';
import './AdminLayout.css';

type NavItem = {
  to: string;
  label: string;
  end?: boolean;
  disabled?: boolean;
  icon: ReactNode;
};

const ADMIN_NAV_MAIN: NavItem[] = [
  { to: APP_ROUTES.platform, label: 'Tổng quan', end: true, icon: <IconDashboard /> },
  { to: APP_ROUTES.platformUsers, label: 'Người dùng', icon: <IconUsers /> },
];

const ADMIN_NAV_UPCOMING: NavItem[] = [
  {
    to: `${APP_ROUTES.platform}/verifications`,
    label: 'Xác minh',
    disabled: true,
    icon: <IconShield />,
  },
  {
    to: `${APP_ROUTES.platform}/reports`,
    label: 'Báo cáo',
    disabled: true,
    icon: <IconFlag />,
  },
];

function renderNavItem(item: NavItem) {
  if (item.disabled) {
    return (
      <span key={item.to} className="adm-nav__link adm-nav__link--disabled" title="Sắp có">
        <span className="adm-nav__icon">{item.icon}</span>
        {item.label}
        <span className="adm-nav__soon">Sắp có</span>
      </span>
    );
  }

  return (
    <NavLink
      key={item.to}
      to={item.to}
      end={item.end}
      className={({ isActive }) => `adm-nav__link${isActive ? ' adm-nav__link--active' : ''}`}
    >
      <span className="adm-nav__icon">{item.icon}</span>
      {item.label}
    </NavLink>
  );
}

type AdminLayoutProps = {
  title: string;
  subtitle?: string;
  children: ReactNode;
};

const userInitials = (displayName: string | undefined, email: string) => {
  const source = displayName?.trim() || email;
  return source
    .split(/\s+/)
    .slice(0, 2)
    .map((part) => part.charAt(0).toUpperCase())
    .join('');
};

export function AdminLayout({ title, subtitle, children }: AdminLayoutProps) {
  const { user } = useAuth();

  return (
    <div className="adm-page">
      <header className="adm-topbar">
        <div className="adm-topbar__inner">
          <AppLogo href={APP_ROUTES.platform} className="adm-topbar__logo" />
          <div className="adm-topbar__actions">
            <NavLink
              to={APP_ROUTES.platformProfile}
              className={({ isActive }) =>
                `adm-profile-btn${isActive ? ' adm-profile-btn--active' : ''}`
              }
              title="Quản lý hồ sơ"
            >
              <span className="adm-profile-btn__avatar">
                {userInitials(user?.displayName, user?.email ?? 'A')}
              </span>
              <span className="adm-profile-btn__text">
                <span className="adm-profile-btn__label">Hồ sơ</span>
                <span className="adm-profile-btn__name">
                  {user?.displayName?.trim() || user?.email || 'Admin'}
                </span>
              </span>
              <IconUser className="adm-profile-btn__icon" />
            </NavLink>
            <Link to={APP_ROUTES.home} className="tcs-btn tcs-btn--ghost tcs-btn--header adm-topbar__home">
              Trang chủ
            </Link>
            <LogoutButton />
          </div>
        </div>
      </header>

      <div className="adm-shell">
        <aside className="adm-sidebar">
          <p className="adm-sidebar__label">Bảng điều khiển</p>
          <div className="adm-sidebar__divider" role="separator" aria-hidden="true" />
          <nav className="adm-nav" aria-label="Quản trị nền tảng">
            {ADMIN_NAV_MAIN.map(renderNavItem)}
          </nav>
          <div className="adm-sidebar__divider" role="separator" aria-hidden="true" />
          <p className="adm-sidebar__label adm-sidebar__label--sub">Sắp có</p>
          <nav className="adm-nav adm-nav--secondary" aria-label="Chức năng sắp có">
            {ADMIN_NAV_UPCOMING.map(renderNavItem)}
          </nav>
        </aside>

        <div className="adm-content">
          <header className="adm-header">
            <div className="adm-header__inner">
              <h1 className="adm-header__title">{title}</h1>
              {subtitle ? <p className="adm-header__subtitle">{subtitle}</p> : null}
            </div>
          </header>
          <main className="adm-main">{children}</main>
        </div>
      </div>
    </div>
  );
}
